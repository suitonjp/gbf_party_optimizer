// 編成対象の武器の取得
function goc_loadWeapons(data, target_ele, fix, slvmax){
  var wpArr = new Array();
...
function getSkillEffect(weapon_name, skill_name, skill_level){
  ...
  // スキル名判定
  switch(skill_name){
    case "スカルパレード": return ["gen_dark_da", da_m(skill_level)];
    case "紅蓮の呪印・弐": return ["guren", atk_b(skill_level)];

    // コラボ系武器（アンノウン枠として統一）
    case "アンノウン・ATK": 
    case "エクストラ・ATK": return ["unknown_atk", atk_m(skill_level)];
    ...
    case "Vスキル":
    case "その魂よ、安らかに":　
    case "炎の背骨": return ["unknown_atk", atk_b(skill_level)];
    case "エクストラ・ATK III": return ["unknown_atk", atk_b2(skill_level)];
    case "エクストラ・ATK IIII": return ["unknown_atk", atk_b3(skill_level)];
    case "ミフネ流剣法・極意": return ["unknown_atk", atk_b(skill_level), "gen_fire_bkn", 0.07];
    case "インテリジェンス":   return ["unknown_atk", atk_b(skill_level), "gen_dark_bkn", 0.07];
    case "スピードスペル": return ["unknown_atk", atk_s(skill_level)];
    case "マジックチャージ": return ["unknown_bob", bob_m(skill_level)];
    case "ミフネ流剣法・双星": 
    case "デクステリティ": return ["unknown_da", da_s(skill_level)];
    // バハ武器
    case "ヒュムアニムス・ウィス":
    case "ドーラアニムス・ウィス":
    case "エルンアニムス・ウィス":
  ...
  if(stype == "gb"){
    if(suf == "II"){
      if(skill_name.slice(-4) == "攻刃II") return [spref+"_atk", atk_b2(skill_level)];
      if(skill_name.slice(-4) == "守護II") return [spref+"_hp",  hp_b2(skill_level)];
    }
    if(suf == "暴君") return [spref+"_atk", atk_b(skill_level), spref+"_bkn", 0.1];
    if(suf == "攻刃") return [spref+"_atk", atk_b(skill_level)];
    if(suf == "守護") return [spref+"_hp",  hp_b(skill_level)];
    if(suf == "背水") return [spref+"_bob", bob_b(skill_level)];
    if(suf == "技巧") return [spref+"_cri", cri_b(skill_level)];
    if(suf == "二手") return [spref+"_da",  da_b(skill_level)];
    if(suf == "三手") return [spref+"_da",  da_b(skill_level), spref+"_ta", ta_b(skill_level)];
  }
  else if(stype == "gm"){
    if(suf == "攻刃") return [spref+"_atk", atk_m(skill_level)];
    if(suf == "守護") return [spref+"_hp",  hp_m(skill_level)];
    if(suf == "背水") return [spref+"_bob", bob_m(skill_level)];
    if(suf == "技巧") return [spref+"_cri", cri_m(skill_level)];
    if(suf == "二手") return [spref+"_da",  da_m(skill_level)];
    if(suf == "刹那") return [spref+"_atk", atk_m(skill_level), spref+"_cri", cri_m(skill_level)];
    if(suf == "羅刹") return [spref+"_atk", atk_m(skill_level)];
    if(suf == "克己") return [spref+"_da",  da_m(skill_level), spref+"_cri", cri_m(skill_level)];
  }
  else if(stype == "gs"){
    if(suf == "攻刃") return [spref+"_atk", atk_s(skill_level)];
    if(suf == "守護") return [spref+"_hp",  hp_s(skill_level)];
    if(suf == "背水") return [spref+"_bob", bob_s(skill_level)];
    if(suf == "技巧") return [spref+"_cri", cri_s(skill_level)];
    if(suf == "二手") return [spref+"_da",  da_s(skill_level)];
    if(suf == "神威") return [spref+"_atk", atk_s(skill_level), spref+"_hp", hp_s(skill_level)];
  }
  else if(stype == "st"){
    if(suf == "渾身") return [spref+"_kns", kns_b(skill_level)];
  }
  else if(stype == "m"){
    suf = skill_name.slice(5)
    if(suf == "攻刃II") return [spref+"_atk", atk_b(skill_level)];
    if(suf == "守護II") return [spref+"_hp",  hp_b_mu(skill_level)];
    if(suf == "背水II") return [spref+"_bob", bob_b(skill_level)];
    if(suf == "攻刃") return [spref+"_atk", atk_m(skill_level)];
